def generate_accept_form(doc_data, supplier_data, company_data, product_data, list_data=None,
    ...
    styles.add(ParagraphStyle(name='Line_Data_Large', fontName='Times', alignment=TA_LEFT, fontSize=12, leading=12))
    ...
    styles.add(ParagraphStyle(name='Table_Data', fontName='Times', fontSize=12, leading=14, alignment=TA_CENTER))
    ...
    styles.add(ParagraphStyle(name='Line_Label_Name', fontName='Times', fontSize=13, leading=6, alignment=TA_LEFT))
    styles.add(ParagraphStyle(name='Line_Label_Center', fontName='Times Bold', fontSize=14, leading=16, alignment=TA_CENTER))
    ...
    if list_data:
        ...
        first_page_pass_limit = 52
        second_page_pass_limit = 120
        sign_size = 15
        ...
        t1 = Table(data1, colWidths=(1.5 * cm, 3.5 * cm, 4.5 * cm, 1.5 * cm, 3.5 * cm), minRowHeights=((.7 * cm,)*(rows+1)))  # , rowHeights = [.3*cm, .5*cm, .3*cm, .5*cm])
        t1.setStyle(TableStyle([
            # ('GRID', (0, 0), (-1, -1), 0.25, colors.black),
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')
        ]))
        ...
        cert = list_data['certificate_number']
        if cert:
            cert = cert.strip()
